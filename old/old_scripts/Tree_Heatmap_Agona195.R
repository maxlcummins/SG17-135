...
pais_df <-
        pais_df %>% filter(perc_coverage > 40) %>% filter(perc_identity > 95)
...
#PAI_genes
pais_df$GENE <- gsub("_NC_003198","",pais_df$GENE)
...
p <- ggtree(tree2, aes(color = group)) %<+%
        Metadata +
        geom_tiplab(size = 1.2, align = TRUE, aes(color = HC5_or_other)) +
...
p2 <- ggtree(tree2) %<+%
        Metadata +
        geom_tiplab(size = 1.2, align = TRUE, aes(color = HC5_or_other), offset = 0.004) +
