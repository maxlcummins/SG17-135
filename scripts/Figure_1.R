...
p <- ggtree(tree, layout = "circular") %<+%
        ...
        #geom_tippoint(size = 0.5,
                      #aes(colour = HC5_or_other),
        #              show.legend = TRUE) +
        geom_tiplab2(size = 1.5, align = TRUE, offset = 0.001, aes(colour = HC5_or_other))
        
#get node labels
#

p2 <- p + geom_tiplab(aes(image = Flag), geom="image", size = 0.010, hjust = 0.5, align = TRUE, offset = -0.0002, linetype = NULL)  + geom_treescale(x = 0.1, y = -2.25, offset = -3, fontsize = 2) 

p2 <- flip(p2, 263, 375)

#p3 <- ggtree(tree2, branch.length =  "none")  %<+% Metadata + geom_text2(size = 2, aes(subset=!isTip, label=node), hjust=-.5) + geom_tiplab(size = 1, aes(colour = HC5_or_other))
...
pdf(file = "new_figures/Figure 1.pdf", paper = "a4")


new_data <- as.data.frame(Metadata[,"Source_Niche"])
#new_data$HC50 <- Metadata$HC50
#new_data$HC50 <- gsub("^","HC50:", new_data$HC50)
#new_data$HC5_or_other <- Metadata$HC5_or_other
rownames(new_data) <- Metadata$working_name
...
pdf(file = "new_figures/Supplementary Figure 1.pdf", paper = "a4")
...
p2 <- ggtree(tree#,
        ...
        geom_text2(size = 2, aes(subset=!isTip, label=node), hjust=-.5) + geom_tiplab(size = 1, aes(colour = HC5_or_other))

#p3 <- ggtree(tree, branch.length =  "none")  %<+% Metadata + geom_text2(size = 2, aes(subset=!isTip, label=node), hjust=-.5) + geom_tiplab(size = 1, aes(colour = HC5_or_other))
